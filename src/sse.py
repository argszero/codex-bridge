...
class SseTranslator:
    """Translates upstream Chat Completions SSE chunks into Responses API SSE events."""
    ...
    def feed(self, chunk: dict) -> str:
        """Process one Chat Completions SSE chunk, return SSE event string(s)."""
        ...
        # --- text content (including refusal) ---
        text_delta = delta.get("content") or delta.get("refusal")
        if text_delta:
            output += self._ensure_started()
            self.content_so_far += text_delta
        ...
        # --- legacy function_call (singular, deprecated) ---
        if delta.get("function_call") and not delta.get("tool_calls"):
            fc = delta["function_call"]
            fc_name = fc.get("name", "")
            fc_id = fc_name or "legacy"
            if fc_id not in self.tool_calls:
                call = {"id": fc_id, "name": fc_name, "arguments": ""}
                self.tool_calls[fc_id] = call
                oi = self.output_item_count
                self.output_item_count += 1
                self.output_items.append({
                    "index": oi,
                ...
                log.info("tool (legacy): " + fc_name)
            call = self.tool_calls[fc_id]
            if fc.get("name"):
                call["name"] = fc["name"]
            d = fc.get("arguments", "")
            if d:
                call["arguments"] += d
            oi = self._item_index(f"fc_{fc_id}")
            if oi >= 0 and d:
                output += self._emit(
                    "response.function_call_arguments.delta",
                    {
    ...
    def done(self, usage_override: Optional[dict] = None) -> str:
        """Finalize the stream: emit done events for all output items, then
        response.completed, then the terminal [DONE] sentinel."""
        output = self._ensure_started()
        ...
        # Build output snapshot for response.completed
        # Pre-index tool calls by itemId for O(1) lookup
